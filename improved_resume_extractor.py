# improved_resume_extractor.py - 改进版简历提取器（含项目经历）
# -*- coding: utf-8 -*-
...
class ImprovedResumeExtractor:
    ...
    def __init__(self):
        ...
        self.project_keywords = {
            ...
            "task": ["作業内容", "担当内容", "業務内容", "実施内容", "開発内容"],
            "system": ["システム", "プロジェクト", "案件", "開発"],
            "phase": [
                "要件定義",
                "基本設計",
                "詳細設計",
    ...
    def extract_from_excel(self, file_path: str) -> Dict:
        ...
        try:
            ...
            result["projects"] = self._extract_projects(all_data)  # 新增项目提取
    ...
    def _extract_projects(self, all_data: List[Dict]) -> List[Dict]:
        """提取项目经历"""
        projects = []
                ...
                for col in range(len(df.columns)):
                    ...
                    if pd.notna(cell):
                        ...
                        if any(
                            ...
                            # 检查是否是列标题行
                            row_text = " ".join(
                                [str(c) for c in df.iloc[idx] if pd.notna(c)]
                            )
                            if (
                                "期間" in row_text
                                or "№" in row_text
                                ...
                                project_start_rows.append(idx)
                                print(
                                    f"    发现项目列表开始行: [{idx}] {row_text[:50]}..."
                                )

            # 方法2: 提取项目信息
            for start_row in project_start_rows:
                # 从标题行的下一行开始提取
                for idx in range(start_row + 1, min(start_row + 20, len(df))):
                    project_info = self._parse_project_row(df, idx)
                    if project_info:
                        projects.append(project_info)
                        print(f"    提取到项目: {project_info['name'][:30]}...")

            # 方法3: 模式匹配（查找包含期间和系统名的行）
            for idx in range(len(df)):
                row = df.iloc[idx]
                row_text = " ".join([str(cell) for cell in row if pd.notna(cell)])

                # 检查是否包含项目信息的特征
                if self._is_project_row(row_text):
                    project_info = self._parse_project_content(df, idx)
                    if project_info and not self._is_duplicate_project(
                        project_info, projects
                    ):
                        projects.append(project_info)
                        print(f"    模式匹配提取项目: {project_info['name'][:30]}...")

        # 按时间排序（最新的在前）
        projects.sort(key=lambda x: self._parse_project_date(x["period"]), reverse=True)

        print(f"    共提取到 {len(projects)} 个项目")
        return projects[:10]  # 返回最近的10个项目

    def _parse_project_row(self, df: pd.DataFrame, row_idx: int) -> Optional[Dict]:
        """解析项目行数据"""
        try:
            row = df.iloc[row_idx]
            ...
            for col_idx, cell in enumerate(row):
                ...
                # 项目编号
                if re.match(r"^\d+$", cell_str) and int(cell_str) < 100:
                    project_no = cell_str

                # 期间（可能是日期对象或字符串）
                elif isinstance(cell, datetime):
                    period = cell.strftime("%Y年%m月")
                elif re.search(r"\d{4}年\d{1,2}月|20\d{2}[-/]\d{1,2}", cell_str):
                    period = cell_str

                # 项目内容（通常是最长的文本）
                elif len(cell_str) > 20 and (
                    "システム" in cell_str or "開発" in cell_str or "業務" in cell_str
                ):
                    content = cell_str

            if content:
                return self._parse_project_details(content, period)
    ...
    def _parse_project_details(self, content: str, period: str = None) -> Dict:
        """解析项目详细信息"""
        project = {
            "period": period or "",
            "name": "",
        ...
        for i, line in enumerate(lines):
            ...
            if "業務概要" in line or "概要" in line:
                # 提取概要内容
                desc_lines = []
                for j in range(i + 1, len(lines)):
                    if "担当" in lines[j] or "作業" in lines[j]:
                        break
                    desc_lines.append(lines[j].strip())
                project["description"] = " ".join(desc_lines)

            elif "担当内容" in line or "作業内容" in line:
                # 提取任务列表
                for j in range(i + 1, len(lines)):
                    task = lines[j].strip()
    ...
    def _is_project_row(self, row_text: str) -> bool:
        """判断是否是项目行"""
        # 项目行特征
        has_period = bool(re.search(r"\d{4}年\d{1,2}月|20\d{2}[-/]\d{1,2}", row_text))
        has_system = any(
            keyword in row_text for keyword in ["システム", "開発", "プロジェクト"]
        )
        has_task = any(keyword in row_text for keyword in ["業務", "担当", "概要"])

        return has_period and (has_system or has_task)

    def _parse_project_content(self, df: pd.DataFrame, row_idx: int) -> Optional[Dict]:
        """从DataFrame中解析项目内容"""
        try:
            # 收集当前行和附近行的内容
            content_parts = []
            ...
            for offset in range(5):
                ...
                row = df.iloc[row_idx + offset]
                for cell in row:
                    if pd.notna(cell):
                        cell_str = str(cell).strip()

                        # 提取期间
                        if not period and re.search(r"\d{4}年\d{1,2}月", cell_str):
                            period = cell_str
    ...
    def _is_duplicate_project(
        self, project: Dict, existing_projects: List[Dict]
    ) -> bool:
        """检查是否是重复项目"""
        for existing in existing_projects:
            # 比较项目名称和期间
            if project.get("name") == existing.get("name") and project.get(
                "period"
            ) == existing.get("period"):
                return True

            # 比较描述的相似度
            if project.get("description") and existing.get("description"):
                if project["description"][:50] == existing["description"][:50]:
                    return True

        return False

    def _parse_project_date(self, period_str: str) -> datetime:
        """解析项目日期用于排序"""
        if not period_str:
        ...
        # 尝试其他格式
        match = re.search(r"20(\d{2})[-/](\d{1,2})", period_str)
        if match:
...
if __name__ == "__main__":
    extractor = ImprovedResumeExtractor()
    ...
    for file in test_files:
        ...
        try:
            ...
            if "error" not in result:
                ...
                print(f"✅ 项目({len(result['projects'])}个):")
                for i, project in enumerate(result["projects"][:3]):
                    print(f"   项目{i+1}: {project['period']} - {project['name']}")
